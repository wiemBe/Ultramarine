...
func handleAddTask(ctx *gin.Context) {
	...
	if !isValidAndReachable(targetPoint) {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": fmt.Sprintf("Station %d is at invalid coordinates (%d,%d) - not reachable", input.Station, targetPoint.X, targetPoint.Y)})
		return
	}
	...
	if err != nil {
		log.Printf("Error fetching created task %d: %v", newID, err)
...
func handleGetTasks(ctx *gin.Context) {
	...
	if err := rows.Err(); err != nil {
		log.Printf("Error after task rows: %v", err)
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "db error"})
		return
	}
...
func handleGetNextTask(ctx *gin.Context) {
	...
	if err != nil {
		log.Printf("Error find next task: %v", err)
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "db error"})
		return
	}
	_, err = tx.Exec("UPDATE tasks SET status = ? WHERE id = ?", StatusAssigned, nextTask.ID)
	if err != nil {
		log.Printf("Error assign task %d: %v", nextTask.ID, err)
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "db error"})
		return
	}
	targetPoint, ok := stationCoords[nextTask.Station]
	if !ok {
		log.Printf("Config Err: Sta %d miss", nextTask.Station)
		tx.Rollback()
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "config error"})
		return
	}
	if !isValid(targetPoint) {
		log.Printf("Config Err: Sta %d invalid map loc", nextTask.Station)
		tx.Rollback()
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "config error"})
		return
	}
	...
	if _, exists := robotStates[nextTask.ID]; exists {
		log.Printf("Warn: Overwriting state Task %d", nextTask.ID)
	}
	robotStates[nextTask.ID] = newState
	robotStatesMutex.Unlock()
	if err := tx.Commit(); err != nil {
		log.Printf("Error commit assign %d: %v", nextTask.ID, err)
		robotStatesMutex.Lock()
	...
	nextTask.Status = StatusAssigned
...
func handleReportStatus(ctx *gin.Context) {
	...
	var input struct {
		...
		LastCmdStatus string `json:"last_cmd_status" binding:"required"`
	}
	if err := ctx.ShouldBindJSON(&input); err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "Invalid input", "details": err.Error()})
		return
	...
	if input.Theta < OrientationNorth || input.Theta > OrientationWest {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "Invalid theta"})
		return
	}
	inputStatusUpper := strings.ToUpper(input.LastCmdStatus)
	if !(inputStatusUpper == StatusInit || inputStatusUpper == StatusDone || inputStatusUpper == StatusObstacle) {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "Invalid last_cmd_status"})
		return
	}
	if input.X < 0 || input.X >= mapWidth || input.Y < 0 || input.Y >= mapHeight {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "Coords out of bounds"})
		return
	}
	...
	if !exists {
		log.Printf("Status report inactive Task %d", taskID)
		ctx.JSON(http.StatusNotFound, gin.H{"error": "Task state not found"})
		return
	}
	...
	case StatusInit:
		...
		state.NeedsPathRecalc = true
		state.CurrentCommandIndex = 0
	case StatusDone:
		if state.PathCommands == nil {
			state.NeedsPathRecalc = true
		} else if state.CurrentCommandIndex < len(state.PathCommands) {
			state.CurrentCommandIndex++
		}
		if !state.NeedsPathRecalc {
			state.NeedsPathRecalc = false
		}
	case StatusObstacle:
		state.NeedsPathRecalc = true
		state.PathCommands = nil
		state.CurrentCommandIndex = 0
		log.Printf("Obstacle Task %d at (%d,%d)", taskID, input.X, input.Y)
	}
...
func handleGetNextCommand(ctx *gin.Context) {
	...
	if !state.IsInitialized {
		ctx.JSON(http.StatusOK, gin.H{"command": CmdNone, "message": "Waiting for initial robot report"})
		return
	}
	...
	if state.PathCommands == nil || state.NeedsPathRecalc {
		...
		if pathErr != nil {
			log.Printf("Pathfind Err Task %d: %v", taskID, pathErr)
			ctx.JSON(http.StatusOK, gin.H{"command": CmdError, "error": fmt.Sprintf("Pathfind fail: %v", pathErr)})
			return
		}
		if len(pathPoints) <= 1 {
			...
			log.Printf("A* Err Task %d: Empty/1pt path", taskID)
			ctx.JSON(http.StatusOK, gin.H{"command": CmdError, "error": "Path not found/unreachable"})
			return
		}
		commands, cmdErr := pathToCommands(pathPoints, state.CurrentOrientation)
		if cmdErr != nil {
			log.Printf("Cmd Gen Err Task %d: %v", taskID, cmdErr)
			ctx.JSON(http.StatusOK, gin.H{"command": CmdError, "error": fmt.Sprintf("Cmd gen fail: %v", cmdErr)})
			return
		}
		log.Printf("Task %d: Gen %d commands.", taskID, len(commands))
	...
	if state.PathCommands == nil {
		log.Printf("Internal Err Task %d: Nil PathCmds", taskID)
		ctx.JSON(http.StatusInternalServerError, gin.H{"command": CmdError, "error": "Internal path error"})
		return
	}
	if state.CurrentCommandIndex < len(state.PathCommands) {
		...
		log.Printf("Task %d: Path finished.", taskID)
		go internalCompleteTask(taskID)
		ctx.JSON(http.StatusOK, gin.H{"command": CmdComplete})
	}
}
...
func handleGetMapState(ctx *gin.Context) {
	...
	response.Map.Width = mapWidth
	response.Map.Height = mapHeight
	response.Map.Grid = worldMap
	response.StationLocs = make(map[string]Point)
	for id, point := range stationCoords {
		response.StationLocs[strconv.Itoa(id)] = point
	}
	response.Robots = make([]RobotInfo, 0)

	robotStatesMutex.RLock()
	activeTaskIDs := make([]int64, 0, len(robotStates))
	for taskID := range robotStates {
	...
	for _, taskID := range activeTaskIDs {
		...
		if state.IsInitialized {
			...
			err := db.QueryRow("SELECT status FROM tasks WHERE id = ?", taskID).Scan(&currentDBStatus)
			if err != nil {
				currentDBStatus = "db_err"
			}
...
func handleCompleteTask(ctx *gin.Context) {
	...
	if err := internalCompleteTask(taskID); err != nil {
		if strings.Contains(err.Error(), "Task state not found") {
			ctx.JSON(http.StatusOK, gin.H{"message": "Task already complete/cleared", "id": taskID})
		} else {
			log.Printf("Err final complete %d: %v", taskID, err)
...
func handleFailTask(ctx *gin.Context) {
	...
	if err := internalFailTask(taskID); err != nil {
		if strings.Contains(err.Error(), "Task state not found") {
			ctx.JSON(http.StatusOK, gin.H{"message": "Task already failed/cleared", "id": taskID})
		} else {
			log.Printf("Err final fail %d: %v", taskID, err)
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		}
		return
	}
...
func internalCompleteTask(taskID int64) error {
	...
	if exists {
		...
		log.Printf("State: Cleaned complete %d", taskID)
	} else {
		log.Printf("State: Not found complete %d", taskID)
	}
	robotStatesMutex.Unlock()
	res, err := db.Exec("UPDATE tasks SET status = ? WHERE id = ? AND status != ?", StatusCompleted, taskID, StatusCompleted)
	if err != nil {
		log.Printf("DB Err complete %d: %v", taskID, err)
		return fmt.Errorf("db error: %w", err)
	}
	rowsAffected, _ := res.RowsAffected()
	if rowsAffected > 0 {
		log.Printf("DB: Marked complete %d", taskID)
	}
	return nil
}
func internalFailTask(taskID int64) error {
	...
	if exists {
		delete(robotStates, taskID)
		log.Printf("State: Cleaned fail %d", taskID)
	} else {
		log.Printf("State: Not found fail %d", taskID)
	}
	robotStatesMutex.Unlock()
	res, err := db.Exec("UPDATE tasks SET status = ? WHERE id = ? AND status != ?", StatusFailed, taskID, StatusFailed)
	if err != nil {
		log.Printf("DB Err fail %d: %v", taskID, err)
		return fmt.Errorf("db error: %w", err)
	}
	rowsAffected, _ := res.RowsAffected()
	if rowsAffected > 0 {
		log.Printf("DB: Marked fail %d", taskID)
	}
	return nil
}
...
func heuristic(a, b Point) float64 {
	// Temel Manhattan mesafesi
	dx := math.Abs(float64(a.X - b.X))
	dy := math.Abs(float64(a.Y - b.Y))

	// Tie-breaker: düz yolları tercih eder
	// 1.0/1000.0 faktörü, A* algoritmasının optimal yoldan sapmamasını sağlar
	crossProduct := math.Abs(float64((a.X - b.X)*(a.Y - b.Y)))
	return dx + dy + crossProduct*0.001
}
func isValid(p Point) bool {
	if p.X < 0 || p.X >= mapWidth || p.Y < 0 || p.Y >= mapHeight {
		return false
	}
	return worldMap[p.Y][p.X] == CellFree
}
func isValidAndReachable(p Point) bool {
	// Check bounds
	if p.X < 0 || p.X >= mapWidth || p.Y < 0 || p.Y >= mapHeight {
		return false
	}
	...
	if worldMap[p.Y][p.X] != CellFree {
		return false
	}

	// Check if it has at least one free neighbor (not completely surrounded)
	neighbors := []Point{
	...
	return false // Completely surrounded by obstacles
}
...
func findPathAStar(start, goal Point) ([]Point, error) {
	if !isValid(start) {
		return nil, fmt.Errorf("start invalid (%d,%d)", start.X, start.Y)
	}
	if !isValid(goal) {
		return nil, fmt.Errorf("goal invalid (%d,%d)", goal.X, goal.Y)
	}
	if start == goal {
	...
	nodeMap := make(map[Point]*aStarNode)
...
func pathToCommands(path []Point, startOrientation int) ([]string, error) {
	...
	for i := 1; i < len(path); i++ {
		...
		if dx == 1 && dy == 0 {
			...
			return nil, fmt.Errorf("invalid step: %v to %v", currentPos, nextPos)
		}
...
func initDB() {
	...
	if err != nil {
		log.Fatalf("FATAL: DB Open error: %v", err)
	}
	if err = db.Ping(); err != nil {
		db.Close()
		log.Fatalf("FATAL: DB Ping error: %v", err)
	}
	_, err = db.Exec("PRAGMA journal_mode=WAL;")
	if err != nil {
		log.Printf("Warn: WAL mode failed: %v", err)
	}
	_, err = db.Exec("PRAGMA busy_timeout = 5000;")
	if err != nil {
		log.Printf("Warn: Busy timeout failed: %v", err)
	}
	if isNewDB {
		...
		if err != nil {
			db.Close()
			log.Fatalf("FATAL: Schema creation error: %v", err)
		}
		log.Println("DB schema created.")
	} else {
		log.Printf("Existing DB found: '%s'.", dbPath)
	}
